...
if mSl==0
    ...
    if aSE==0  %% for evergreen
        Sl_min = Sl;
        Sl=Sl+(0.5*Sl_min)*exp(-AgeL./(0.5*age_cr));%(1-AgeL./dmg)*(1.5*Sl_min)*(AgeL<dmg); % hard-coded here to be fixed in the future
        LAI = Sl*B(1); %% Leaf area index for green biomass
    end

    if aSE==1  %% for crops
        Sl=Sl+(1-AgeL./dmg)*(1.5*Sl_min)*(AgeL<dmg);
        LAI = Sl*B(1); %% Leaf area index for green biomass
    end
