...
##                             -> model_run/<ST>/era5_land/Meteo_<ST>_1985_2020.mat
##
...
## Writes: $TC_INPUT_DATA/meteo/Meteo_<ST>_raw.mat   (STAGING, intermediates)
##         $MODEL_RUN/<ST>/era5_land/Meteo_<ST>_1985_2020.mat   (the forcing)
##
## The finished file goes into the run tree, one level above the fixed_lma/dyn_lma
## pair that shares it, so model_run is self-contained. Stage 1 routes it via a
## dest_dir field in each raw file; stage 2 obeys it. Raw files are intermediates:
##     find $TC_INPUT_DATA/meteo -name 'Meteo_*_raw.mat' -delete
##
...
ls -lh "$MODEL_RUN"/*/era5_land/Meteo_*_"$YEAR_TAG".mat 2>/dev/null | head
