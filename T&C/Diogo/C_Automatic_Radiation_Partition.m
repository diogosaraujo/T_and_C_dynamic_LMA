...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function[SD,SB,SAD1,SAD2,SAB1,SAB2,PARB,PARD,N,Rsws,t_bef,t_aft]=C_Automatic_Radiation_Partition(Date,Lat,Lon,Zbas,DeltaGMT,Pr,Tdew,Rsw,GRAPH_VAR,t_bef_force,t_aft_force)
...
[Anno,Mese,Giorno,Ora,Min,~]=datevec(D);
% datet = datetime(D,'convertfrom','datenum');
% [Anno,Mese,Giorno] = ymd(datet);
% [Ora,Min,~] = hms(datet);
Ora= Ora + (Min/30>1);
...
for IS=1:length(T_BF)
    ...
    LWP0 = zeros(size(Tdew));
    beta_A = zeros(size(Tdew));
    for m = 1:12
        ix = Mese==m;
        if sum(ix)>0
            LWP0(ix)=Pc.LWP_R(m);
            beta_A(ix)=Pc.beta_A(m);
        end
    end


    [SB,SD,~,~,~,~,~,~]=ComputeRadiationForcingsN(Datam,DeltaGMT,Lon,Lat,...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if isnan(t_bef_force) % otherwise not needed
    [SB,SD,~,~,~,~,~,~]=ComputeRadiationForcingsN(Datam,DeltaGMT,Lon,Lat,...
        LWP0,N,Zbas,Tdew,beta_A,alpha_A,omega_A1,omega_A2,uo,un,rho_g,t_bef,t_aft);
end
...
N=Nsim;
N(isnan(Rsw))=0.15;
N(isnan(N))=mean(N,'omitnan');

%%%%%%%%%%%%%%%%%%%
%%%

[SB,SD,SAB1,SAB2,SAD1,SAD2,PARB,PARD]=ComputeRadiationForcingsN(Datam,DeltaGMT,Lon,Lat,...
        LWP0,N,Zbas,Tdew,beta_A,alpha_A,omega_A1,omega_A2,uo,un,rho_g,t_bef,t_aft);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function[Nsim]=Estimate_CloudCover(Rsw,Rsws,Pr,~)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[h_S,E0] = SetSunVariables_light(Datam,DeltaGMT,Lon,Lat,t_bef,t_aft);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
ix = Ntmp > 0;
if sum(ix)>0
    [EB1_t,EB2_t,ED1_t,ED2_t] = SetCloudySkyRadiation(LWP(ix),h_S(ix),EB1(ix),EB2(ix),Edp1(ix),Edp2(ix),N(ix),rho_s1(ix),rho_s2(ix),rho_g);
    EB1(ix) = EB1_t;
    EB2(ix) = EB2_t;
    ED1(ix) = ED1_t;
    ED2(ix) = ED2_t;

    %[EB1(ix),EB2(ix),ED1(ix),ED2(ix)]
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%   Subfunction SetSunVariables   %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [h_S,E0] = SetSunVariables_light(Datam,DeltaGMT,Lon,Lat,t_bef,t_aft)
%%% INPUT
%%% Datam %% [Yr, MO, DA, HR]
%%% DeltaGMT [°]
%%% Lon [°]
...
% end
n = size(Datam, 1);
jDay = day(datetime(Datam(:,1),Datam(:,2),Datam(:,3),Datam(:,4),zeros(n,1),zeros(n,1)),'dayofyear');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%t_bef=0.5;%
%t_aft=0.5;%
t= (Datam(:,4)) + (-t_bef:0.0166666:t_aft);

% tau_S = zeros(size(t));
% tic
% tau_S(t<(12 + Delta_TSL)) = 15*pi/180*(t(t<(12 + Delta_TSL)) + 12 - Delta_TSL);
% tau_S(t>=(12 + Delta_TSL)) = 15*pi/180*(t(t>=(12 + Delta_TSL)) - 12 - Delta_TSL);
% toc
% tic
Tlim = (12 + Delta_TSL);
tau_S = 15*pi/180*(t - sign(t-Tlim)*12 - Delta_TSL);
% tau_S(t >= Tlim) = 15*pi/180*(t(t >= Tlim) - 12 - Delta_TSL);
% for i=1:length(t)
...
% if(h_S > 0.0) %% if there is sunshine
    ...
    T1 = [TO1 TR1 TG1 TW1 TA1 TN1];
    T2 = [TO2 TR2 TG2 TW2 TA2 TN2];
    % T1 = zeros(1, 6);
    % T2 = zeros(1, 6);
    % 
    % T1(1)  = TO1;
...
% if(h_S > 0) % if there is sunshine
    ...
    for j=1:4  %%% check TDB + TDIR <= 1
        ix = SDB_DIR(:, j) > 1;
        if sum(ix)>0
        TDIR(ix, j) = TDIR(ix, j)./SDB_DIR(ix, j);
        TDB(ix, j) = TDB(ix, j)./SDB_DIR(ix, j);
        end
    ...
    EDp2     = (1 - N).*Edp2 + N.*sum(EDp2_all,2);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% Sky albedo for cloudy sky
    rho_csb1 = (1-N).*rho_s1 + N.*RDIR(:,1)*k(1)/K(1);
    rho_csd1 = (1-N).*rho_s1 + N.*RDIF(:,1)*k(1)/K(1);
    rho_csb2 = (1-N).*rho_s2 + N.*sum(RDIR(:,2:4).*k(2:4)./K(2:4),2);
    rho_csd2 = (1-N).*rho_s2 + N.*sum(RDIF(:,2:4).*k(2:4)./K(2:4),2);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
