...
CLAMPED = []          # (Psan, Pcla) of every layer texture_pair had to adjust


def texture_pair(layer):
    """Sand and clay fractions, rounded as emitted and guaranteed to leave silt.

    Soil_parameters derives silt as `Psil = 1-Psan-Pcla` and errors on `if Psil<0`.
    US-SP2 and US-SP4 are reported as sand+clay = 1 exactly (no silt), where that
    subtraction lands on -1e-17 rather than 0 and the run died at parameter setup
    (array job 35717, 4 runs). The values are rounded to the emitted precision
    FIRST and clamped after, because clamping the full-precision number and then
    rounding for output can round the sum straight back up over 1.

    SILT_FLOOR is far above double precision and far below the resolution of any
    texture source, so it fixes the arithmetic without changing the soil.

    The rounding is done with the SAME .5g the emitter has always used, not a
    wider format, so a soil that already leaves silt is emitted byte-identically
    to before. Only the degenerate profiles change, which keeps MOD_PARAM for the
    stations that have already run reproducing their stored results exactly.
    """
    SILT_FLOOR = 1e-4                          # 0.01% silt; 5 sig figs resolve 1e-5
    r = lambda x: float(f"{float(x):.5g}")     # exactly what gets written out
    psan, pcla = r(layer["Psan"]), r(layer["Pcla"])
    total = psan + pcla
    if total > 1.0 - SILT_FLOOR:
        scale = (1.0 - SILT_FLOOR) / total
        CLAMPED.append((psan, pcla))
        psan, pcla = r(psan * scale), r(pcla * scale)
    return psan, pcla
...
def soil_block(layers, kbot):
    ...
    ms = len(layers)
    tex = [texture_pair(l) for l in layers]
    psan = ", ".join(f"{s:.5g}" for s, _ in tex)
    pcla = ", ".join(f"{c:.5g}" for _, c in tex)
...
def main() -> int:
    ...
    if clamped_stations:
        print(f"silt clamp applied (sand+clay reported as 1, no silt left for "
              f"Soil_parameters):")
        for sid, n in clamped_stations:
            print(f"  ~ {sid:<8} {n} layer(s) adjusted by <=1e-4")
        print()
    else:
