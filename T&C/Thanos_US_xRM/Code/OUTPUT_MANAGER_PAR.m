...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if t==2
    tit{1}=strcat(out_fol,'OUTPUT_',TITLE_SAVE,'_AVG.dat');
    fid(1)=fopen(tit{1},'a');
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if t==2
    tit4{1}=strcat(out_fol,'OUTPUT_',TITLE_SAVE,'_STD.dat');
...
if OPT_VAR_SM_ON == 1 
    ...
    if t==2
        tit8{1}=strcat(out_fol,'OUTPUT_',TITLE_SAVE,'_VAR_SM.dat');
    ...
    for ijki=1:cc_max
        for ievc=1:length(EVcode)
            if Ccrown_OUT(ievc,ijki)>0
                tit2{ievc,ijki}=strcat(out_fol,'OUTPUT_',TITLE_SAVE,'_AVG_PFT_',num2str(ijki),'_code_',num2str(EVcode(ievc)),'.dat');
...
if t==2
    for ijki=1:cc_max
        for ievc=1:length(EVcode)
            if Ccrown_OUT(ievc,ijki)>0
                tit3{ievc,ijki}=strcat(out_fol,'OUTPUT_',TITLE_SAVE,'_STD_PFT_',num2str(ijki),'_code_',num2str(EVcode(ievc)),'.dat');
...
if  length(intersect(t,tstore))==1 ||  t==N_time_step
    Title_save = strcat(out_fol,'OUTPUT_',TITLE_SAVE,'_SPATIAL_',num2str(t));
...
for ipo=1:npoint
    ...
    if t==2
        tit5{1,ipo}=strcat(out_fol,'OUTPUT_',TITLE_SAVE,'_PIXEL_',num2str(ij),'.dat');
    ...
    %%%%%%%%%
    if t==2
        tit6{ipo}=strcat(out_fol,'OUTPUT_',TITLE_SAVE,'_PIXEL_SOIL_',num2str(ij),'.dat');
    ...
    if t==2
        for ijki=1:cc_max
            tit7{1+ijki,ipo}=strcat(out_fol,'OUTPUT_',TITLE_SAVE,'_PIXEL_',num2str(ij),'_PFT_',num2str(ijki),'.dat');
...
if not(isempty(RES_ID_List))
    for ire=1:length(RES_ID_List)
        if t==2
            tit8{1,ire}=strcat(out_fol,'OUTPUT_',TITLE_SAVE,'_RESERVOIRS_',num2str(RES_ID_List(ire)),'.dat');
